...
import gridfs
import pymongo
def connect_to_modularforms_db(host='localhost',port=37010,collection='',create=True):
    r"""
    Return a handle to a modular forms database or a specific collection.

    """
    try:
        C = pymongo.connection.Connection('{0}:{1}'.format(host,port))
...
from web_char import WebCharacter
